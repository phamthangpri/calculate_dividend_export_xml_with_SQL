		 ...
		 + (CASE WHEN products IN ('ECO18C-ECO18D','ECO18D-ECO18C') 
			...
			+(CASE WHEN has_ecc = 1 AND has_ecd = 0 
			   ...
			   WHEN has_ecc = 1 AND has_ecd = 1 AND ir = 0
					THEN 'T9'
				END		)	
		 +'_' + CAST(c.associateId AS VARCHAR)																AS filename_eco --cr�er 2 champs pour ne pas embrouiller la revue
	INTO ##ASSOCIATES_AGG
	FROM ##CONTRATS c
	JOIN -------------Get text data ------------
		(SELECT associateId
...
-----------Table agr�g�e par associate, par produit ----------------
SET @t1 = GETDATE();
BEGIN
